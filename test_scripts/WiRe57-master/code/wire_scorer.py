...
def main():
    ...
    all_predictions = json.load(open("../data/WiRe57_extractions_by_ollie_clausie_openie_stanford_minie_reverb_props-export.json"))
...
def split_tuples_by_extractor(gold, tuples):
    '''
        gold = {s['id'] : s['tuples'] for doc in reference.values() for s in doc}
        gold: dict_keys(['TO 1', 'TO 2', 'TO 3', 'TO 4', 'TO 5', 'TO 6', 'TO 7', 'TO 8', 'TO 9', 'TO 10', 'TO 11', 'TO 12', 'TO 13', 'TO 14', 'TO 15',
        ...
        t:  {'arg1': 'The third', 'rel': 'is', 'arg2': 'sudden market losses', 'extractor': 'ollie', 'score': 0.7460666499114321}

    '''
    tuples = parse_t5_result()
    systems = sorted(list(set(t['extractor'] for st in tuples.values() for t in st)))
    predictions_by_OIE = {e : {} for e in systems}
    # print("gold:", gold)
    for s in gold:
        # print("s: ", s)
        # print("tuples[s]: ", tuples[s])
        for t in tuples[s]:
...
def parse_t5_result(result_file="/mnt/data/jinzhihong/projects/cycle/generation/one_test_generation/text2data0116(cyclefrombase)_wp_test_on_wire57_t5.tsv"):
    import csv
    import re

    title_ids = ['TO 1', 'TO 2', 'TO 3', 'TO 4', 'TO 5', 'TO 6', 'TO 7', 'TO 8', 'TO 9', 'TO 10', 'TO 11', 'TO 12', 'TO 13', 'TO 14', 'TO 15',
        'FI 1', 'FI 2', 'FI 3', 'FI 4', 'FI 5', 'FI 6', 'FI 7',
        'CH 1', 'CH 2', 'CH 3', 'CH 4', 'CH 5', 'CH 6', 'CH 7', 'CH 8', 'CH 9', 'CH 10',
        'EM 1', 'EM 2', 'EM 3', 'EM 4', 'EM 5', 'EM 6', 'EM 7', 'EM 8', 'EM 9',
        'CE 1', 'CE 2', 'CE 3', 'CE 4', 'CE 5', 'CE 6', 'CE 7', 'CE 8', 'CE 9', 'CE 10', 'CE 11', 'CE 12', 'CE 13', 'CE 14', 'CE 15', 'CE 16']
    ...
    tuples = {} # dict {title_id : extractions}
    with open(result_file, mode='r', encoding='utf-8') as f:
        ...
        for i, row in enumerate(list(reader)):
            ...
            # get title_id
            if 1 <= num <= 15:
                title_id = f"TO {str(num)}"
            elif 16 <= num <= 22:
                title_id = f"FI {str(num - 15)}"
            elif 23 <= num <= 32:
                title_id = f"CH {str(num - 22)}"
            elif 33 <= num <= 41:
                title_id = f"EM {str(num - 32)}"
            elif 42 <= num <= 57:
                title_id = f"CE {str(num - 41)}"

            # get extractions
            # extractions = {}
            if "<then>" in row[1]:
                extractions = row[1].split('<then>')
            else:
                extractions = [row[1]]
            for extraction in extractions:
                # print("data: ", data)
                pattern = r"subject <is> (.*) <and> relation <is> (.*) <and> object <is> (.*)"
                searchobj = re.search(pattern, extraction, re.M | re.I)
                if searchobj != None:
                    subject = searchobj.group(1)
                    relation = searchobj.group(2)
                    object = searchobj.group(3)
                    t = {'arg1': subject, 'rel': relation, 'arg2': object, 'extractor': 't5_cyclefrombase', 'score': 0.666}
                    tuples[title_id] = tuples.get(title_id, []) + [t]
                    # print(f"now tuples[{title_id}]: ", tuples[title_id])
    return tuples
