...
from test_scripts.CaRB.oie_readers.t5SepReader import T5SepReader, BenchieResultReader
...
from test_scripts.CaRB.oie_readers.goldReader import GoldReader
from test_scripts.CaRB.matcher import Matcher
...
def carb_eval(epoch=1, eval_loss=1, benchmark="carb", dataset_type="dev", cycle_dataset_name="lsoie_validation", generation_filepath=os.path.join(cycle_training_rootpath, 'generation/text2data.generations1202')):

    predicted = T5SepReader()
    predicted.read(generation_filepath)
    ...
    if benchmark == 'wire57':
        b = Benchmark(os.path.join(cycle_training_rootpath, f"test_scripts/CaRB/data/gold/CaRB-W.tsv"))
    elif benchmark == "carb":
        b = Benchmark(os.path.join(cycle_training_rootpath, f"test_scripts/CaRB/data/gold/{dataset_type}.tsv"))
    else:
        b = Benchmark(os.path.join(cycle_training_rootpath, "test_scripts/CaRB/data/gold/benchie_gold.tsv"))

    out_filename = os.path.join(cycle_training_rootpath, "test_scripts/CaRB/my_outputs/log.dat")
    ...
    auc, optimal_f1_point = b.compare(predicted=predicted.oie,
                            ...
                            error_file=os.path.join(cycle_training_rootpath, 'test_scripts/CaRB/my_outputs/errorlog'),
                            # binary=args["--binary"]
                            binary=False)

    print("Epoch: {} AUC: {}\t Optimal (precision, recall, F1): {}".format(epoch, auc, optimal_f1_point))

    metric_log_dir = os.path.join(cycle_training_rootpath, f'generation/eval_log/cycle_{cycle_dataset_name}')
    if not os.path.exists(metric_log_dir):
        os.makedirs(metric_log_dir)
    # logfile2write = os.path.join(metric_log_dir, generation_filepath.split('/')[-1] + f"_eval_on_{benchmark}")
    with open(os.path.join(metric_log_dir, generation_filepath.split('/')[-1] + f"_eval_on_{benchmark}_{dataset_type}"), mode="a", encoding="utf-8") as f:
        f.write("Epoch: {}\tEval Loss: {}\tAUC: {}\t Optimal (precision, recall, F1): {}\n".format(epoch, eval_loss, auc, optimal_f1_point))

    print((auc, optimal_f1_point))  # (0.286, array([0.369, 0.419, 0.392]))
    return auc, optimal_f1_point



def eval_benchie_predictions(generation_filepath=os.path.join(cycle_training_rootpath, "test_scripts/benchie/data/oie_systems_explicit_extractions/clausie_explicit.txt")):
    predicted = BenchieResultReader()
    predicted.read(generation_filepath)
    matchingFunc = Matcher.binary_linient_tuple_match
    out_filename = os.path.join(cycle_training_rootpath, "test_scripts/CaRB/my_outputs/log.dat")
    b = Benchmark(os.path.join(cycle_training_rootpath, "test_scripts/CaRB/data/gold/benchie_gold.tsv"))
    auc, optimal_f1_point = b.compare(predicted=predicted.oie,
                                      ...
                                      output_fn=out_filename,
                                      # error_file = args["--error-file"],
                                      error_file=os.path.join(cycle_training_rootpath,
                                                              'test_scripts/CaRB/my_outputs/errorlog'),
                                      # binary=args["--binary"]
                                      binary=False)
...
if __name__ == '__main__':
    ...
    # predicted = T5SepReader()
    # predicted.read("/home/langchao/projects_jzh/cycle_training/generation/text2data.generations1202")
    # # b = Benchmark(args['--gold'])
    # # out_filename = args['--out']
    #
    #
    # matchingFunc = Matcher.binary_linient_tuple_match
    # b = Benchmark("data/gold/test.tsv")
    # out_filename = "my_outputs/log.dat"
    #
    # logging.info("Writing PR curve of {} to {}".format(predicted.name, out_filename))
    #
    # auc, optimal_f1_point = b.compare(predicted = predicted.oie,
    ...
    #                         output_fn = out_filename,
    #                         # error_file = args["--error-file"],
    #                         error_file = "my_outputs/errorlog",
    #                         # binary = args["--binary"]
    #                           binary = False)
    #
    # print("AUC: {}\t Optimal (precision, recall, F1): {}".format( auc, optimal_f1_point ))

    # carb_eval(cycle_dataset_name="lsoie_validation")
    # carb_eval(epoch=1, eval_loss=1,  benchmark="carb", dataset_type="test", cycle_dataset_name="normal_sft", generation_filepath=os.path.join(cycle_training_rootpath, 'generation/one_test_generation/text2data0111_normal_sft_test_on_carb_test_t5.csv'))

    carb_eval(epoch=1, eval_loss=1,  benchmark="wire57", dataset_type="test", cycle_dataset_name="normal_sft", generation_filepath=os.path.join(cycle_training_rootpath, 'generation/one_test_generation/text2data0116(cyclefrombase)_bp_test_on_wire57_t5.tsv'))
    carb_eval(epoch=1, eval_loss=1,  benchmark="wire57", dataset_type="test", cycle_dataset_name="normal_sft", generation_filepath=os.path.join(cycle_training_rootpath, 'generation/one_test_generation/text2data0116(cyclefrombase)_wp_test_on_wire57_t5.tsv'))

    # eval_benchie_predictions(generation_filepath=os.path.join(cycle_training_rootpath, "test_scripts/benchie/data/oie_systems_explicit_extractions/m2oie_en_explicit.txt"))